...
import { getTokenExpiresAt, hashToken, signToken } from './tokenService.js';
...
async function issueSessionToken(payload) {
  const token = signToken(payload);

  await createSession({
    subjectId: payload.sub,
    subjectType: payload.role,
...
export async function loginCustomer({ identifier, password }) {
  ...
  const token = await issueSessionToken({ sub: customer.id, role: 'customer', name: customer.name });
...
export async function loginAdmin({ email, password }) {
  ...
  const token = await issueSessionToken({ sub: admin.id, role: admin.role, name: admin.name });
  return {
    token,
